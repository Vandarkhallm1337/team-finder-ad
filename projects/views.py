...
from .constants import PAGINATE, STATUS_CLOSE_LOWER
...
class ProjectCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_success_url(self):
        return reverse(
            "projects:project-detail", kwargs={"id": self.object.id}
        )
...
class ProjectUpdateView(
    LoginRequiredMixin, OwnerOrAdminRequiredMixin, UpdateView
):
    model = Project
    form_class = ProjectForm
    template_name = "projects/create-project.html"
...
def toggle_participate(request, id):
    ...
    if request.user == project.owner:
        return JsonResponse(
            {"status": "owner_cannot_join"}, status=HTTPStatus.BAD_REQUEST
        )
...
def complete_project(request, id):
    ...
    return JsonResponse({"status": "ok", "project_status": STATUS_CLOSE_LOWER})
